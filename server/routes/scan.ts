...
// Fetch Detections
router.get('/detections', authenticateToken, async (req: AuthRequest, res) => {
  const user = req.user!;
  const tenantId = user.tenantId;
...
router.get('/history', authenticateToken, async (req: AuthRequest, res) => {
  ...
  try {
    const { rows } = await query(
      'SELECT * FROM scans WHERE tenant_id = $1 ORDER BY created_at DESC LIMIT 50',
      [user.tenantId]
    );
    res.json({ success: true, data: rows });
  } catch (err: any) {
...
router.post('/history', authenticateToken, async (req: AuthRequest, res) => {
  ...
  try {
    await query(
      `INSERT INTO scans (tenant_id, created_by, input_query, risk_level, similarity, confidence, spread_level, total_links) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
      [user.tenantId, user.uid, inputQuery, riskLevel, similarity, confidence, spreadLevel, totalLinks]
    );
...
// AI Proxies
router.post('/ai/fingerprint', authenticateToken, async (req: AuthRequest, res) => {
  try {
...
router.post('/ai/analyze', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const response = await ai.models.generateContent({
      model: 'gemini-2.5-flash',
      contents: `Analyze this content and its live search scan results. Determine the severity, spread risk, likely ownership of the IP, and business impact. Content: ${content}. Scanner Info: Found ${scanData.total_found} hits across the web. URLs: ${scanData.results.map((r: any) => r.url).join(', ')}`,
      config: {
        responseMimeType: "application/json",
        responseSchema: schema,
      }
...
router.post('/ai/actions', authenticateToken, async (req: AuthRequest, res) => {
  try {
