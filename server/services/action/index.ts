...
export const getAbuseContact = async (domain: string) => {
  try {
    const apiKey = process.env.WHOIS_API_KEY;
    if (!apiKey) return 'abuse@mock-isp.com';
...
// Send DMCA Notice
export const sendDMCANotice = async (detectionId: string, tenantId: string) => {
...
`
  ...
  await query(
    'INSERT INTO actions(tenant_id, detection_id, action_type, status, result_details) VALUES($1, $2, $3, $4, $5)',
    [tenantId, detectionId, 'DMCA_TAKEDOWN', 'EXECUTED', `Email sent to ${abuseEmail}`]
  );
