...
export const runWebScan = async (searchQuery: string, tenantId: string) => {
    ...
    try {
      ...
      if (hasVideo) {
        ...
        searchTerms.forEach(term => {
          if (contentText.includes(term)) matchCount++;
        });
        ...
        if (similarity > 50) {
          ...
          // Store in DB
          await query(
            'INSERT INTO detections(id, tenant_id, url, title, risk_level, confidence, status, created_at) VALUES($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT DO NOTHING',
            [detection.id, detection.tenantId, detection.url, detection.title, detection.riskLevel, detection.confidence, detection.status, detection.createdAt]
          );
        }
      }
    } catch (e: any) {
...
export const startLiveMonitor = async (streamUrl: string, tenantId: string) => {
