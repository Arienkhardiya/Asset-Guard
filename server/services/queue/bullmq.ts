...
// Stub types for when Redis is disabled
type StubQueue = { add: (...args: any[]) => Promise<any> };

let scanQueue: Queue | StubQueue;
let actionQueue: Queue | StubQueue;
let initializeWorkers: () => any;
...
if (process.env.REDIS_ENABLED === 'true') {
  ...
  initializeWorkers = () => {
    const scanWorker = new Worker('scanQueue', async (job: Job) => {
  ...
  const noop = async (..._args: any[]) => ({ id: 'mock-job' });
  scanQueue = { add: noop };
